    ...
    <div id="view-tab" class="sub-tab-content active">
        <div class="search-box">
            <div class="search-options">
                ...
                <input type="text" id="searchValue" placeholder="Enter customer ID or name..." onkeypress="handleKeyPress(event)">
                <button onclick="searchCustomer()"><i class="fas fa-search"></i> Search</button>
                <button onclick="showAllCustomers()"><i class="fas fa-users"></i> Show All Customers</button>
                <button onclick="clearResults()"><i class="fas fa-eraser"></i> Clear</button>
            </div>
        </div>
        
        <div id="result"></div>
        <div id="allCustomers" class="customer-list"></div>
    </div>
    
    <!-- Add Customer Tab -->
    <div id="add-tab" class="sub-tab-content">
        <div class="form-container">
            <h3 style="color: #0F4C81; margin-bottom: 20px;"> Add New Customer</h3>
            ...
            <form id="addCustomerForm">
                ...
                <button type="button" onclick="addCustomer()"><i class="fas fa-save"></i> Save Customer</button>
                <button type="button" onclick="clearAddForm()"><i class="fas fa-undo-alt"></i> Clear</button>
            </form>
        </div>
    </div>
</div>
...
<script>
    ...
    function searchCustomer() {
        ...
        document.getElementById('allCustomers').innerHTML = '';
        
        let isNumeric = /^\d+$/.test(searchValue);
        let searchType = isNumeric ? 'id' : 'name';
        
        fetch(`get_customer.php?searchType=${searchType}&searchValue=${encodeURIComponent(searchValue)}`)
        ...
        .then(data => {
            ...
            let html = `
                <div class="customer-info">
                    ...
                    <p><strong><i class="fas fa-id-card"></i> ID:</strong> ${data.customer.Cid}</p>
                    <p><strong><i class="fas fa-user"></i> Name:</strong> ${data.customer.CName}</p>
                    <p><strong><i class="fas fa-phone"></i> Phone:</strong> ${data.customer.TelNo || 'N/A'}</p>
                    <p><strong><i class="fas fa-map-marker-alt"></i> Address:</strong> ${data.customer.Address || 'N/A'}</p>
                </div>
                ...
                <table class="order-table">
                    <thead>
                        <tr>
                            ...
                            <th><i class="fas fa-shipping-fast"></i> Shipper</th>
            ...
            if(data.orders.length === 0) {
                html += `<tr><td colspan="5" style="text-align:center;"><i class="fas fa-inbox"></i> No orders found for this customer</td></tr>`;
            } else {
                data.orders.forEach(order => {
                    let date = order.InvDate ? new Date(order.InvDate).toLocaleDateString() : 'N/A';
                    html += `<tr>
                        <td>${order.InvNo}</td>
                        <td>${date}</td>
                        <td>${order.EmpName}</td>
                ...
                <table style="width:100%; border-collapse: collapse;">
                    <thead>
                        <tr>
                            ...
                            <th style="border:1px solid #ccc; padding:8px; background:#0F4C81; color:white;"> Phone</th>
                            <th style="border:1px solid #ccc; padding:8px; background:#0F4C81; color:white;"> Address</th>
                            <th style="border:1px solid #ccc; padding:8px; background:#0F4C81; color:white;"> Orders</th>
                            <th style="border:1px solid #ccc; padding:8px; background:#0F4C81; color:white;"> Total Spent</th>
                        </tr>
                    </thead>
                    <tbody>`;
            data.forEach(cust => {
                html += `<tr>
                    <td>${cust.Cid}</td>
                    <td>${cust.CName}</td>
                    <td>${cust.TelNo || 'N/A'}</td>
                    <td>${cust.Address || 'N/A'}</td>
                    <td>${cust.OrderCount}</td>
                    <td>$${parseFloat(cust.TotalSpent).toFixed(2)}</td>
                </tr>`;
            });
            html += `</tbody>
                </table>`;
    ...
    function addCustomer() {
        ...
        if(!name) {
            document.getElementById('addMessage').innerHTML = '<div class="error"><i class="fas fa-exclamation-circle"></i> Please enter customer name</div>';
            return;
        }
        ...
        fetch('add_customer.php', {
            ...
            body: JSON.stringify({CName: name, TelNo: phone, Address: address})
        })
        .then(response => response.json())
        .then(data => {
